...
- Status elementu (`proposed`/`ready`/`in-progress`/`blocked`/`done`) → **pole „Etap"** na tablicy (`Backlog`/`Gotowe`/`W toku`/`W przeglądzie`/`Zrobione`/`Zablokowane`) — jedyne miejsce statusu (nie dubluj statusu w etykietach).
