...
const ALL_ACTIONS: readonly ItemAction[] = ["edit", "accept", "reject", "trash", "restore", "preview"];
...
describe("allowedActions — mapowanie stan → dozwolone akcje (S-13 F3)", () => {
  ...
  it.each(["rejected", "deleted"] as const)("%s → podgląd / przywróć (read-only + kosz dwukierunkowo)", (status) => {
    expect([...allowedActions(status)].sort()).toEqual(["preview", "restore"]);
  });
});
