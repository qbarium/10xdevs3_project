...
- **Przywrócenie z kosza cofa ostatnią tranzycję w wymiarze akceptacji** (🔁 Decyzja projektowa 2026-06-16) — item przeniesiony do kosza wraca akcją „przywróć z kosza": `deleted → accepted` (zachowując stan operacyjny) oraz `rejected → pending` (ponowne wejście do bramy walidacji). Jedyne nieodwracalne usunięcie to globalna akcja „wyczyść kosz". Stan operacyjny zadania (`nowe` / `w realizacji` / `zrealizowane` / `anulowane`) jest niezależny od stanu akceptacji i wzajemnie przechodni.
...
- Item odrzucony dostępny w widoku Kosz; można go przywrócić z Kosza akcją „przywróć z kosza" do „Elementy do akceptacji" (`rejected → pending`, ponowne wejście do bramy walidacji — 🔁 Decyzja projektowa 2026-06-16).
...
- **🔁 Decyzja projektowa 2026-06-15:** akcja dostępna dla itemów **wszystkich typów** (rozszerzenie z pierwotnego `task`-only); etykiety stanu per-typ; `zrealizowane`/obsłużone dowolnego typu → widok Zakończone.
...
- FR-004: User może wprowadzić wsad jako drag-and-drop pliku audio (`.mp3`, `.wav`). Pliki przekraczające limit rozmiaru (FR-019) są odrzucane z komunikatem, bez prób chunkingu. Każdy plik = osobna sesja importu (zgodnie z FR-015). Priority: nice-to-have → **poza MVP (V2)** — decyzja 2026-07-20 (Open Question 2 rozstrzygnięte NIE).
...
- FR-019: Limit rozmiaru pliku audio (`.mp3`, `.wav`): 25 MB (limit usługi transkrypcji audio dostawcy AI). UI blokuje upload większych plików przed submitem z komunikatem. Priority: nice-to-have → **poza MVP (V2)** — decyzja 2026-07-20 (Open Question 2 rozstrzygnięte NIE).
...
- FR-015: Każdy submit (jeden element wsadu — paste tekstu LUB jeden plik tekstowy LUB jeden plik audio) tworzy nową sesję importu z unikalnym identyfikatorem. Sesja zawiera: rejestr wejścia (dla paste — treść wklejonego tekstu zapisana bezpośrednio w sesji; dla pliku tekstowego lub audio — referencja do pliku, którego zawartość przechowywana jest w storage serwera i jest nierozerwalnie związana z sesją), status sesji (`przetwarzanie` / `zakończona z itemami` / `zakończona bez itemów` / `niepowodzenie`), liczbę wygenerowanych itemów lub komunikat błędu. Sesja importu **nie jest już filtrem widoków jednolitej listy** (🔁 Decyzja projektowa 2026-06-20, nadpisuje wcześniejsze „filtr dodatkowy" z FR-008); elementy danej sesji są dostępne w osobnym widoku master-detail w dzienniku sesji importu (wycinek S-10, rozszerzenie FR-027). Widok walidacyjny domyślnie nie filtruje po sesji. Sesje zachowywane bez TTL (audit log; polityka cleanup poza MVP). Powiązanie itemu z sesją (`import_session_id`) jest utrzymywane, dopóki sesja istnieje; jej ewentualne usunięcie świadomie czyści to powiązanie (`set null`) i sam item zostaje — link item↔sesja to pomocniczy (best-effort) audit trail, nie więź nierozerwalna (referencja do pliku w storage pozostaje związana z sesją i ginie wraz z nią). Priority: must-have
...
- FR-027: User ma dostęp do osobnego widoku **dziennika sesji importu** — chronologicznej listy sesji z minimalnym zakresem informacji: rejestr wejścia (treść wklejonego tekstu lub nazwa i typ pliku), łączny status sesji (`przetwarzanie` / `zakończona z itemami` / `zakończona bez itemów` / `niepowodzenie`), liczba wygenerowanych itemów lub komunikat błędu, akcja „spróbuj ponownie" dostępna dla sesji ze statusem `niepowodzenie` (wsad zachowany — brak konieczności wprowadzania od nowa). Per-file rozbicie → poza MVP. **🔁 Decyzja projektowa 2026-06-20:** podgląd zawartości itemów w obrębie sesji **wchodzi do zakresu** jako widok master-detail w dzienniku sesji (wycinek S-10) — zniesienie wcześniejszego „poza MVP"; operacje reużywają dialogu edycji (S-05) i przenoszenia do kosza (S-06), a elementy w Koszu pozostają tylko do odczytu (FR-011). Priority: must-have
...
- FR-008: Itemy są widoczne na **jednej jednolitej liście z trzema warstwami filtrowania**. (1) **Filtr główny** (widok nawigacyjny, single-select) przełącza między rozłącznymi zbiorami: **Aktywne** (`accepted`, operacyjnie otwarte) / **Zakończone** (`accepted`, zamknięte operacyjnie) / **Anulowane** (zadania w stanie `anulowane`) / **Elementy do akceptacji** (`pending`) / **Kosz** (`rejected` + `deleted`). (2) **Filtr typu** (rząd akcji w widoku, single-select): Wszystkie / Zadania / Notatki / Pomysły / Decyzje / Inne — eliminuje potrzebę osobnych zakładek per typ. (3) **Filtry dodatkowe** w obrębie aktualnego widoku: sortowanie i filtrowanie po dacie utworzenia, dacie modyfikacji oraz tytule; wyszukiwanie po tytule i opisie; filtr po statusie operacyjnym tam, gdzie widok zawiera więcej niż jeden status. **🔁 Decyzja projektowa 2026-06-20:** sesja importu **przestaje być filtrem dodatkowym listy** — wejście „sesja → jej elementy" realizuje osobny widok master-detail w dzienniku sesji importu (wycinek S-10, rozszerzenie FR-027); powód: filtr po sesji nie skaluje się (sesji mogą być tysiące) i nie należy do warstwy filtrów listy. Rozróżnienie poprzedniego statusu w Koszu (`rejected` vs `deleted`) realizuje **etykieta na elemencie**, nie filtr (zgodnie z FR-012, 🔁 2026-06-19). Mitygacja nadużywania typu `other` przez klasyfikator realizowana na poziomie promptu (definicja, kiedy `other` jest właściwy + monitoring udziału `other` jako sygnał diagnostyczny) — szczegóły w spec technicznej. Priority: must-have
...
- FR-009: Zadania (typ `task`) mają **stan operacyjny** o czterech wzajemnie przechodnich wartościach: `nowe` / `w realizacji` / `zrealizowane` / `anulowane`. User może w każdej chwili zmienić stan operacyjny zadania (w tym cofnąć `zrealizowane` do `w realizacji` lub `nowe`). Stan operacyjny i stan akceptacji to dwa niezależne wymiary — przeniesienie do kosza zmienia tylko stan akceptacji; stan operacyjny zachowany przez cały cykl życia itemu. Brak stanu operacyjnie finalnego dopóki item nie jest w koszu. Akcje zmieniające stan operacyjny dostępne zarówno per item, jak i jako akcje zbiorcze w ujednoliconym modelu z FR-007. **🔁 Decyzja projektowa 2026-06-15:** stan operacyjny obejmuje **wszystkie typy** itemów (rozszerzenie pierwotnego ograniczenia do `task`); etykiety stanu są per-typ (`zrealizowane`: zadanie „Zrobione", notatka „Obsłużona", decyzja „Podjęta", pomysł „Obsłużony"); `done`/obsłużone dowolnego typu → widok Zakończone. Koszt: backfill `operational_status NULL→'new'` + zmiana RPC `persist_classification` (plan zmiany `task-operational-lifecycle`). Pierwotne brzmienie („itemy nie-`task` nie mają stanu operacyjnego") zastąpione tą decyzją. Priority: must-have
...
- FR-012: Item odrzucony w stagingu otrzymuje status akceptacji `rejected` i znika z widoku walidacyjnego (filtr „Elementy do akceptacji"). User MA dostęp do odrzuconych itemów przez **filtr główny Kosz** (FR-008); poprzedni status (`rejected` vs `deleted`) jest **oznaczony na każdym itemie etykietą (badge)**, a zawężanie w obrębie widoku odbywa się — jak w pozostałych widokach — **wyłącznie filtrem typu** (bez osobnego filtra statusu — 🔁 Decyzja projektowa 2026-06-19). Item `rejected` opuszcza stan odrzucenia **albo** przez przywrócenie z Kosza (`rejected → pending`, ponowne wejście do bramy walidacji — FR-013), **albo** trwale przez akcję „wyczyść kosz" (FR-016) (🔁 Decyzja projektowa 2026-06-16). Priority: must-have
...
- FR-013: User może przenieść zaakceptowany item do kosza (status akceptacji: `accepted` → `deleted`); akcja dostępna per item i jako akcja zbiorcza w ujednoliconym modelu z FR-007. Stan operacyjny itemu zachowany przy przeniesieniu (model dwóch niezależnych wymiarów — patrz FR-009). User może **przywrócić item z kosza** akcją „przywróć z kosza", dostępną per item i zbiorczo. Przywracanie cofa ostatnią tranzycję wymiaru akceptacji i jest **dwukierunkowe** (🔁 Decyzja projektowa 2026-06-16): `deleted → accepted` — item wraca dokładnie do poprzedniego stanu operacyjnego (zadanie zakończone → po przywróceniu znów zakończone); `rejected → pending` — item odrzucony wraca do „Elementy do akceptacji" (ponowne wejście do bramy walidacji). Item zaakceptowany przeniesiony do kosza nie trafia do `pending` (wraca do `accepted`); item odrzucony przywraca się do `pending`. Trwałe usunięcie wyłącznie przez „wyczyść kosz" (FR-016). Priority: must-have
...
- **Audio jako wsad wejściowy (FR-004, FR-019).** Drag-and-drop plików `.mp3`/`.wav` + transkrypcja przez usługę dostawcy AI — **poza MVP** (decyzja 2026-07-20, Open Question 2 rozstrzygnięte NIE). Wymaga nowej osi infrastruktury (transkrypcja + walidacja magic-bytes) niewspółmiernej do budżetu zaliczeniowego → V2. W MVP user transkrybuje audio zewnętrznie i wkleja tekst; wejście MVP = paste lub plik tekstowy `.txt`/`.md`.
...
2. **Czy audio jako wsad wejdzie do MVP (FR-004 nice-to-have)?** — **ROZSTRZYGNIĘTE 2026-07-20: NIE.** Audio (FR-004, FR-019) pozostaje **poza MVP** i przechodzi do V2. Powód: transkrypcja audio to nowa oś infrastruktury (integracja usługi transkrypcji dostawcy AI + walidacja magic-bytes), która nie dowozi proporcjonalnej wartości w budżecie zaliczeniowym; user z dyktafonem transkrybuje nagranie zewnętrznie i wkleja tekst (akceptowalny kompromis MVP, zgodny z notą Sokratesa przy FR-004). Wejście MVP = paste tekstu lub plik tekstowy `.txt`/`.md`.
