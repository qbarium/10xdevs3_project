...
## „Naprawione" dla dup-React SSR = brak re-optymalizacji na zimnym starcie, nie pojedynczy udany render

- **Context**: Astro `output:"server"` + wyspy React + Vite (dev). Diagnoza/naprawa „Invalid hook call / more than one copy of React" widocznego TYLKO w `npm run dev`, na stronach z wyspami (np. `/import-sessions`, `/items`). Konfiguracja `astro.config.mjs`. Powiązane: lekcja „Bug widoczny tylko w `dev`…" (dup-React SSR, S-08) oraz S-10 `session-items-detail` follow-up F5 (`context/changes/session-items-detail/follow-ups/review-fixes.md`).
- **Problem**: `resolve.dedupe` + `ssr.noExternal` dla `react`/`react-dom` NIE zapobiegają mid-session re-optymalizacji Vite. Późno odkrywany dep (tu `astro/env/runtime`) wyzwala „optimized dependencies changed. reloading", co rozjeżdża generacje `?v=` Reacta (core vs `react-dom/server`) w jednym renderze SSR → „more than one copy of React". To wyścig: crash łapie tylko render „w poprzek" reloadu, więc pojedynczy udany render (albo zielony build / zimny render) fałszywie ogłasza „naprawione" (S-10: kryterium 3.8 było odhaczone `[x]`, a bug wciąż żył).
- **Rule**: Dla tej klasy buga kryterium „naprawione" to BRAK mid-session re-optymalizacji na zimnym starcie (brak „optimized dependencies changed. reloading"), a nie pojedynczy udany render. Eliminuj sam trigger: pre-bundluj późno odkrywany dep przez `optimizeDeps.include` + `ssr.optimizeDeps.include` (tu `astro/env/runtime`) — nie polegaj wyłącznie na `dedupe` + `ssr.noExternal`. Weryfikuj zimnym startem (odstaw `.vite`/`.astro`, BEZ `--force` — `--force` tłumi re-optymalizację i daje fałszywy zielony wynik).
- **Applies to**: implement, impl-review, research

## Dup-React SSR: przypnij CAŁĄ populację późnych depów wyspy, nie jeden — weryfikuj z dialogiem i trasą API

- **Context**: Astro `output:"server"` + wyspy React + Vite (dev), adapter Cloudflare/workerd. S-12 (`dup-react-ssr-dev-fix`) — trzecie podejście do „Invalid hook call / more than one copy of React" na `/import-sessions` i `/items*` w `npm run dev`. Rozwinięcie §„Bug widoczny tylko w dev…" i §„«Naprawione» = brak reopt…".
- **Problem**: S-10 przypiął jeden późny dep (`astro/env/runtime`) i zweryfikował `reopt_fired=0` — ale środowisko `deps_ssr` (render wysp) trzymało CAŁĄ populację ~15 depów jako późno odkrywane (rodzina React + `@supabase/ssr`, `zod`, `sonner`, `lucide-react`, `radix-ui`, `@radix-ui/react-slot`, `class-variance-authority`, `clsx`, `tailwind-merge`). Dowolny z nich odkryty w trakcie sesji bumpuje `browserHash` i rozjeżdża generacje `?v=` (`react` core vs `react-dom/server`) → dwie instancje Reacta. `reopt_fired=0` z S-10 był fałszywie zielony, bo hammer nie pokrywał ścieżki dialogu (`zod` przez statyczne importy `EditItemDialog`/`AddItemDialog`) ani trasy API z `zod` — mierzył „nie złapaliśmy reopt", nie „wyeliminowaliśmy populację".
- **Rule**: Dla tej klasy buga (a) przypinaj **całą populację** późno-odkrywanych depów osiągalnych z grafu wyspy w `ssr.optimizeDeps.include`, nie pojedynczy dep — split zachodzi przy DOWOLNEJ reopt, więc pin jednego jest konieczny, ale niewystarczający. (b) Weryfikuj **dwutorowo**: behawioralnie — zimny start bez `--force`, sesja pokrywająca middleware + render wysp + **dialog (`zod`)** + **trasę API z `zod`**, kryterium zero `optimized dependencies changed. reloading`; oraz strukturalnie — `deps_ssr/_metadata.json` pokazuje pin na POCZĄTKU `optimized` (nie w ogonie 4800+) i żaden `_metadata.json` nie jest przepisany po cold-scanie (mtime ze startu = zero mid-session reopt). (c) Utrzymanie: nowy dep osiągalny z grafu wyspy → dopisz do `ssr.optimizeDeps.include`. Na workerd `deps_astro` nie niesie Reacta jako chunków `?v=` (`noExternal` wbudowuje go w graf) → parytet top-level `optimizeDeps.include` zbędny; ścieżką crashu jest wyłącznie `deps_ssr`.
- **Applies to**: implement, impl-review, research, plan, plan-review

## Nie używaj top-level `return` we frontmatterze `.astro` — redirect należy do middleware

- **Context**: Strony `.astro` (`output:"server"`) w tym repo, lintowane przez `astro-eslint-parser` + `@typescript-eslint`. Konkretnie: poprawka F2 w `src/pages/items.astro` (paginacja poza zakresem), wychwycone w `/10x-impl-review` S-13 (session-entries-mode).
- **Problem**: Dodanie pierwszego top-level `return` (np. `return Astro.redirect(...)`) we frontmatterze `.astro` wywołało crash reguły `@typescript-eslint/no-misused-promises` („Non-null Assertion Failed: Expected node to have a parent", `checkReturnStatement`) — `astro-eslint-parser` nie daje węzłowi `return` rodzica, którego reguła oczekuje. To crash reguły, więc `// eslint-disable` NIE pomaga. Build (Rollup) tego nie łapie — wyłapuje tylko ESLint. Repo omijało pułapkę nieświadomie, bo wszystkie redirecty żyją w `src/middleware.ts`.
- **Rule**: Nigdy nie używaj top-level `return` (w tym `return Astro.redirect(...)`) we frontmatterze `.astro`. Redirecty rób w `src/middleware.ts`; korekty stanu strony (np. cofnięcie paginacji poza zakresem) rób bez `return` — koryguj kryteria/dane w miejscu. Po każdej edycji `.astro` uruchom `npm run lint` (build nie wykryje tego crashu).
- **Applies to**: implement, impl-review, research, plan
